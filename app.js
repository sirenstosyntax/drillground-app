...
// A predicate rather than a boolean: a pack's gate depends on which area it
// belongs to, and there are two — general material and promotion-prep material.
...
function resourceBanner(d) {
  ...
  if (locked) {
    html += '<p class="resource-lock-note">Part of the resource packs. Unlock to open it.</p>';
  }
  html +=
    '<a class="' +
...
function resourcesPrompt() {
  ...
  return (
    ...
    "<h2>Decks, case studies, and worksheets</h2>" +
    "<p>Most drills tell you to build something first — a thirty-card deck, an answer key, a " +
    "blind-timeline template. The resource packs supply it, so the reps start straight away.</p>" +
...
function renderResources() {
  ...
  html += '<h2 class="view-title">Drill material</h2>';
  html +=
...
function resourceCard(pack, locked) {
  ...
  html +=
    ...
    esc(pack.title) +
    "</span>" +
    (locked ? '<span class="tag lock">Locked</span>' : '<span class="tag">Free</span>') +
    "</div>";
...
function lockedPackBody(pack) {
  ...
  var promo = resources().packPaidArea(pack) === "promotion_prep_packs";
  html +=
    '<section class="section callout lock-note"><h2>Part of the ' +
    (promo ? "promotion-prep material" : "resource packs") +
    "</h2><p>" +
    (promo
      ? "The promotion-prep material is its own unlock — the question decks, the oral-board probes and " +
        "rubrics, the escalation sets. "
      : "One purchase unlocks the material for every drill in the library — the decks, the case " +
        "studies, the worksheets, the reference cards, and the floor plans. ") +
    "The drills themselves are unaffected: every one of the 87 is free and still runnable without " +
    "any of this, because each drill already tells you what to build.</p></section>";
  return html;
}
...
function loadResources() {
  ...
  fetch(RESOURCES_URL, { cache: "no-cache" })
    ...
    .then(function (published) {
      state.resources = resources().indexPacks(published);
      state.resourcesReady = true;
      router();
      // One payload per area, fetched only for the areas the user holds.
      return Promise.all(
        resources().PAID_AREAS.filter(resourcesEntitled).map(loadUnlockedResources)
      );
    })
    .catch(function () {
      // No packs available. The app is complete without them.
      state.resourcesReady = true;
    });
}
...
function loadUnlockedResources(area) {
  return fetch(resourcesUnlockedUrl(area), { cache: "no-cache" })
    ...
    .catch(function () {
      // Entitled but the payload did not arrive. The packs stay previewed, which
      // isLocked() already reports correctly — better than rendering empty packs.
    });
}
