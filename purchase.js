...
// SKU per entitlement area. These strings must match the in-app product ids
// created in Play Console EXACTLY; a typo is a purchase flow that opens and
// then fails with an unhelpful store error. They are managed (non-consumable)
// products, not subscriptions — see monetization.md.
var SKUS = {
  resource_packs: "drillground.resource_packs.v1",
  promotion_prep_packs: "drillground.promotion_prep_packs.v1",
};
...
//
// This is a launch blocker, not a detail. It is safe to ship this module with
// GATE_OPEN true, because then nothing is ever bought.
// ---------------------------------------------------------------------------
...
function createPurchases(deps) {
  ...
  function grant(area) {
    ...
    if (next[area] === true) return false; // already held; nothing to write
    next[area] = true;
    if (d.writeEntitlements) d.writeEntitlements(next);
    if (d.onUnlock) d.onUnlock(area);
    return true;
  }
